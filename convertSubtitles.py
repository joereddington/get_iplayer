#!/usr/bin/env python
#from https://github.com/GetiPlayerAutomator/get-iplayer-automator/blob/master/ttml2srt.py
...
def ttml2srt(ttml_files, srt_ignore_colors, stdout):
    parser = TTML2SRTParser()
    for ttml_file in ttml_files:
        ...
        ttml_fp.close()
        srt_file = os.path.splitext(ttml_file)[0] + '_converted.srt'
        if stdout:
            srt_fp = sys.stdout
        else:
            srt_fp = codecs.open(srt_file, 'w', 'utf_8', 'ignore')
